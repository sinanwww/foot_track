...
class SubstitutionDialog {
  static Future<void> show({
    ...
    final substitutedOut = (match.substitutions ?? []).map((sub) => sub['outPlayerKey'] as String).toSet();
    ...
    if (redCount > 0 || yellowCount >= 2) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('This player has been sent off and cannot be substituted'),
          behavior: SnackBarBehavior.floating,
          margin: EdgeInsets.only(bottom: 200, left: 20, right: 20),
          backgroundColor: Colors.red,
          duration: Duration(seconds: 2),
    ...
    final availablePlayers = (bench ?? []).where((key) => !substitutedOut.contains(key)).toList();
    String? selectedPlayerKey;

    await showDialog<void>(
      context: context,
      builder: (dialogContext) => AlertDialog(
        title: const Text('Select Substitute Player'),
        content: StatefulBuilder(
          builder: (context, setState) => DropdownButton<String>(
            isExpanded: true,
            hint: const Text('Choose player from bench'),
            value: selectedPlayerKey,
            items: availablePlayers.map((playerKey) {
